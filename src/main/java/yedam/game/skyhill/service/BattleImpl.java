...
public class BattleImpl implements Battle {
	...
	public void battleStart(EnemyVO enemyvo,UserVO uservo) {
		...
		while(enemyvo.get체력() > 0) {
			...
			System.out.printf("\t\t\t데미지: %.1f ~ %.1f  데미지 : %.1f ~ %.1f 데미지 : %.1f ~ %.1f" ,
					(최소데미지*0.7),(최소데미지*1.49),
					(기본데미지*0.7),(기본데미지*1.49),
					(최대데미지*0.7),(최대데미지*1.49));
			System.out.println();
	...
	public void attack(double 데미지, double 명중률,EnemyVO enemyvo,UserVO uservo,int count,int select) {
		...
		if (ran.nextInt(100) < 명중률) { //명중률 확률 
			...
			if(ran.nextInt(100) < uservo.get치명타확률()) {
				System.out.println("크리티컬 히트!!");
				System.out.printf("\t\t\t\t"+enemyvo.get이름()+"에게 %.1f 의 피해를 입혔습니다!",데미지*1.4);
				enemyvo.set체력(enemyvo.get체력()-데미지*1.4);
			} else {
				
				System.out.printf("\t\t\t\t"+enemyvo.get이름()+"에게 %.1f 의 피해를 입혔습니다!",데미지);
			...
			if(ran.nextInt(100) < (uservo.get속도()*2) && count == 0) { //추가공격
